...
parser = argparse.ArgumentParser(description = 'CONTAINERNET')
...
def main():
    ...
    vlb.cmd("tcpdump -i vlb-eth2 -s 0 -w vlb-eth2.pcap &")
    vlb.cmd("tcpdump -i vlb-eth3 -s 0 -w vlb-eth3.pcap &")
    ...
    def wget_parallel_loop():
        ...
        while not wget_stop_event.is_set():
            for name, host in hosts:
                ...
                host.cmd("wget -S -O /dev/null http://10.0.2.1:81/ 2>&1 &")
    ...
    def iperf_parallel_loop():
        ...
        while not iperf_stop_event.is_set() and round_number < max_rounds:
